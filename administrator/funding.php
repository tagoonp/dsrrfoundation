                            ...
                            <div class="card">
                              ...
                              <div class="card-block">
                                <table class="table table-bordered table-striped table-vcenter js-dataTable-full" style="font-size: 0.85em;">
                                    ...
                                    <tbody>
                                      <?php
                                      $strSQL = sprintf("SELECT * FROM tb_funding WHERE 1  ");
                                      $resultFund = $db->select($strSQL,false,true);

                                      if($resultFund){
                                        $c = 1;
                                        foreach ($resultFund as $value) {
                                          ?>
                                          <tr>
                                              <td class="text-center"><?php print $c; ?></td>
                                              <td>
                                                <?php
                          												print $value['fund_short_name'];
                          											?>
                                              </td>

                                              <td>
                                                <?php
                          												print $value['fund_full_name'];
                          											?>
                                              </td>
                                              <td class="hidden-xs">
                                                <?php
                          												print $value['fund_contact_address'];
                          											?>
                                              </td>
                                              <td class="text-center">
                                                  <div class="btn-group">
                                                      <a href="nameprefix.php?fund_id=<?php print $value['fund_id']; ?>">
                                                        <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Edit"><i class="ion-edit"></i></button>
                                                      </a>
                                                      <a href="Javascript: delete_a('controller/deleteprefix.php?fund_id=<?php print $value['fund_id']; ?>')">
